...
QUnit.module('Integration | @private | event phase', {
  integration: true,
  beforeEach() {
    igniter = new Igniter();
  },
...
QUnit.test(`We can schedule into the Event Phase via 'sync'`, function(assert) {
...
QUnit.test(`We can schedule into the Event Phase via 'actions'`, function(assert) {
...
QUnit.test(`The Event Phase flushes as a microtask`, function(assert) {
...
QUnit.test(`The Actions queue is flushed after the Sync queue`, function(assert) {
  let advanceTest = assert.async(2);
